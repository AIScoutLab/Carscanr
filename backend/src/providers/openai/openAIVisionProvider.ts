...
const vehicleVisionInstructions = [
  ...
  "First extract any visible badge text, make text, model text, trim text, any readable 4-digit year text, and emblem or logo clues.",
  "If visible text includes a 4-digit model year, keep that year inside the extracted visible text fields and let it influence likely_year.",
  "If readable badge or model text is visible on the vehicle, anchor on that evidence before using body-shape guessing.",
  "If the image contains readable overlay text or marketing text such as a visible year, make, model, or trim, treat that text as high-priority evidence rather than a weak hint.",
  "If the image clearly shows a 4-digit year together with readable make/model overlay text, treat that combination as the displayed vehicle identity unless the text is clearly unrelated to the pictured vehicle.",
  "For clear marketing or internet listing images, do not downgrade a readable overlay year to an older visual-only guess without a very strong visual contradiction.",
  "When overlay text states exact year + make + model, copy those values into the visible text fields and let likely_year, likely_make, and likely_model follow that text by default.",
  "When readable year, make, and model text agree with each other, let that dominate the identification unless the text is obviously unrelated to the vehicle itself.",
