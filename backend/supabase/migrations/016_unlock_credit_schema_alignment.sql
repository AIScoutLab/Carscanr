...
begin
  ...
  if exists (
    ...
    return query select
      ...
      greatest(balance.unlock_credits, 0);
    return;
  end if;

  if balance.free_unlocks_used >= balance.free_unlocks_total and balance.unlock_credits <= 0 then
    return query select
      false,
      false,
      false,
  ...
  update public.user_unlock_balances
  set unlock_credits = greatest(unlock_credits - 1, 0),
      updated_at = now()
  where user_id = p_user_id
    and unlock_credits > 0
  returning free_unlocks_used, unlock_credits
  into balance.free_unlocks_used, balance.unlock_credits;
